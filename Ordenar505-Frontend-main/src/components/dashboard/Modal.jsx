...
const Modal = ({ setIsTableModalOpen }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const parsedData = {
      table_no: parseInt(tableData.table_no, 10),
      seats: parseInt(tableData.seats, 10),
    };

    if (isNaN(parsedData.table_no) || isNaN(parsedData.seats)) {
      enqueueSnackbar("Please enter valid numbers", { variant: "error" });
      return;
    }

    tableMutation.mutate(parsedData);
  };

  const handleCloseModal = () => {
    setTableData({ table_no: "", seats: "" }); // reset state
